...
2. Em "propose_action", preencha platformCampaignId, platformAdId E platformAdSetId com os 3 ids reais do passo 1 (mesmo quando a proposta so precisaria de um deles pra sair de "precisa de mais dados") - a execucao real mais tarde pode exigir o adSetId tambem dependendo do tipo/plataforma, e so descobrir isso na hora de executar vira "falha tecnica" evitavel. Inclua tambem em metricsJson pelo menos um campo real de spend/ctr/cpc/cpl/cpa/conversions do passo 1. EXCECAO pra PAUSE_AD/ACTIVATE_AD quando o usuario pede pra pausar/ativar A CAMPANHA INTEIRA (nao um anuncio especifico dela): nesse caso NAO invente/escolha um anuncio qualquer de dentro da campanha pra preencher platformAdId - deixe platformAdId (e platformAdSetId) de fora e preencha so platformCampaignId. O sistema pausa/ativa a campanha inteira quando so o campaignId vem preenchido, e o anuncio especifico quando o adId vem preenchido - os dois fazem coisas diferentes de verdade na plataforma, entao a escolha de qual preencher tem que refletir exatamente o que o usuario pediu (campanha inteira vs um anuncio dentro dela).
...
const CHAT_ACTION_GUIDANCE = `Alem de "propose_action", numa conversa real com o usuario voce tambem tem: "get_proposal" (ler os detalhes completos de uma proposta especifica, ex: quando o usuario vem de uma notificacao ou menciona uma proposta), "confirm_and_execute_action" (pra uma acao NOVA ja confirmada: cria a proposta E JA EXECUTA de verdade, numa chamada so), "resolve_proposal" (pra uma proposta que JA EXISTE: decide E JA EXECUTA quando aprovada, numa chamada so), "adjust_proposal" (editar titulo/acao/orcamento de uma proposta pendente), e "delete_proposal" (apagar de vez uma proposta que o usuario nao quer mais ver - so falha se ela ja tiver sido executada, que fica como historico real).
